...
# Load data chunks
loadChunkedData <- function(prefix, chunks = 4) {
  data_list <- list()
  for(i in 1:chunks) {
    chunk_file <- sprintf("%s_chunk%d.rds", prefix, i)
    data_list[[i]] <- readRDS(chunk_file)
  }
  # Combine chunks
...
cubesEN <- loadChunkedData("statcan_cubesEN_with_embeddings")
cubesFR <- loadChunkedData("statcan_cubesFR_with_embeddings")
...
server <- function(input, output, session) {
  # Define your secret password (you can load this from an env var if needed)
   # You can make this an env var with Sys.getenv("MY_APP_PASSWORD")
  ...
  user_embedding <- eventReactive(input$submit, {
    req(input$query)
    suppressWarnings(as.numeric(unlist(embed(substring(input$query,1,140), openai))))
  })
  ...
  # CANSIM_ID = cubes$cansimId[top_indices],
